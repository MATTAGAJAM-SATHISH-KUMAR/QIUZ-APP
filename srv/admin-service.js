...
const { CsvExporter, CsvImporter } = require('./handlers/lib/csv-handler');
const { CertificateGenerator } = require('./handlers/lib/certificate-generator');
const { AIService } = require('./handlers/lib/ai-service');
...
module.exports = cds.service.impl(async function () {
  ...
  const {
    ...
    Certificates, AIGenerationRequests, Users
  ...
  this.on('createQuizVersion', async (req) => {
    ...
    if (currentVersion) {
      const snapshot = await buildVersionSnapshot(currentVersion.ID);
  ...
  this.on('importQuestions', async (req) => {
    ...
    const tenantId = req.user?.tenant || 't1';
  ...
  this.on('generateQuizSummary', async (req) => {
    ...
    setImmediate(async () => {
      try {
        const analytics = await computeAnalytics(quizId);
  ...
  this.on('generateCertificate', async (req) => {
    ...
    const user = await SELECT.one.from(Users).where({ ID: attempt.user_ID });
  ...
  async function computeAnalytics(quizId) {
  ...
  async function buildVersionSnapshot(versionId) {
