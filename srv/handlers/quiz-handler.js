...
class QuizServiceHandler extends cds.ApplicationService {
  async init() {
    const db = await cds.connect.to('db');
    const {
      Quizzes, QuizVersions, Questions, QuestionOptions, MatchPairs,
      QuizAttempts, AttemptAnswers, QuizAssignments, LiveSessions, LiveParticipants
    ...
    this.on('startAttempt', async (req) => {
      ...
      if (quiz.tenantId && userTenant && quiz.tenantId !== userTenant) {
    ...
    this.on('resumeAttempt', async (req) => {
      ...
      if (quiz.timeLimitMinutes) {
        ...
        if (elapsed > quiz.timeLimitMinutes * 60) {
          // Auto-submit
          return await autoSubmit(attempt, quiz, db, scoring);
        }
      }
    ...
    this.on('saveAnswer', async (req) => {
      ...
      if (quiz.timeLimitMinutes) {
        const elapsed = (Date.now() - new Date(attempt.startedAt).getTime()) / 1000;
        if (elapsed > quiz.timeLimitMinutes * 60) {
          await autoSubmit(attempt, quiz, db, scoring);
    ...
    this.on('submitAttempt', async (req) => {
      ...
      const quiz = await SELECT.one.from(Quizzes).where({ ID: attempt.quiz_ID });
      const result = await gradeAndFinalize(attempt, quiz, db, scoring);
    ...
    // --- Internal helper: grade and finalize an attempt ---
    async function gradeAndFinalize(attempt, quiz, db, scoring) {
      const { Questions: Q, QuestionOptions: QO, AttemptAnswers: AA, QuizAttempts: QA } = db.entities('quiz.app');

      const questions = await SELECT.from(Q).where({ quizVersion_ID: attempt.quizVersion_ID });
      const answers = await SELECT.from(AA).where({ attempt_ID: attempt.ID });
      ...
      for (const question of questions) {
        ...
        const options = await SELECT.from(QO).where({ question_ID: question.ID });
        ...
        await UPDATE(AA).where({ ID: answer.ID }).set({
      ...
      await UPDATE(QA).where({ ID: attempt.ID }).set({
        status: 'graded',
    ...
    async function autoSubmit(attempt, quiz, db, scoring) {
      return await gradeAndFinalize(attempt, quiz, db, scoring);
    }
  }
}
  ...
  for (const q of questions) {
    ...
    questionViews.push({
      ...
      options: options.map(o => ({
        ...
        mediaUrl: o.mediaUrl
        // isCorrect NEVER sent to client
      })),
...
module.exports = { QuizServiceHandler };
